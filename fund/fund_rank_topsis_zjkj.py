...
def generate_topsis_score(df, col, zb_lst, direc, w):
    ...
    for idx, group in fundrank_input.groupby('M_O_Date'):
        print(idx)
        tmp = group.copy()
        tmp = topsis(tmp, INDICATORS_AFTER_SELECT
                     , direc, w)
        if num == 0:
            out = tmp.copy()
        else:
...
if __name__=='__main__':
      ...
      for idx, group in fund_value_bench.groupby('code'):           
        ...
        for p in month_period:
            ...
            if len(fund_benchmark) >= p * 20 * 0.8:
                try:
...
# ============================================================================
      ...
      INDICATORS_AFTER_SELECT = ['ir_6', 'yearsharp_12', 'ir_12'
            , 'rd_12', 'down_std_24', 'ir_24'
            , 'avgretrace_36', 'ir_36']
      ...
      for i in  underlying_asset_type_id:
          print(i)
          tmp = df[df['underlying_asset_type_id']==i].copy()
      ...
      fund_rank_r.to_csv('result/fund_rank_by_topsis.csv',encoding='gbk')

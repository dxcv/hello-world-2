...
#      fund_lst=fund_id.main_code.drop_duplicates().tolist()
      ...
      for idx,group in df.groupby('code'):
            ...
            res.append(IR_)
            ret.append(res)
      fund_ret=pd.DataFrame(ret,columns=['code','annROR','over_annROR','maxRetrace','avgRetrace','Var','yearsharpRatio','SortinoRatio','IR'])
      ...
      for i in underlying_asset_type_id:
          ...
          if i != 402003:
              ...
              total=len(fund_ret_get)
              fund_ret_get=fund_ret_get.assign(r_rank=100*(total-fund_ret_get['annROR'].rank(ascending=False)+1)/total) \
                                 .assign(v_1_rank=100*(total-fund_ret_get['avgRetrace'].rank(ascending=True)+1)*2/total)\
                                 .assign(v_2_rank=100*(total-fund_ret_get['Var'].rank(ascending=True)+1)*0.5/total)\
                                 .assign(v_3_rank=100*(total-fund_ret_get['maxRetrace'].rank(ascending=True)+1)*1/total)\
                                 .assign(q_rank=100*(total-fund_ret_get['IR'].rank(ascending=False)+1)*1/total)\
                                 .assign(rank=lambda df:df.r_rank+df.v_1_rank+df.v_2_rank+df.v_3_rank+df.q_rank)
        
              fund_ret_get=fund_ret_get.sort_values(by='rank',ascending=False)
              ...
              fund_ret_get.to_excel('result/fund_mark_all_'+str(i)+'.xls',encoding='gbk')   
          else:
              fund_ret_get=fund_ret.query("annROR>0.07 & annROR<0.2")
              total=len(fund_ret_get)
              fund_ret_get=fund_ret_get.assign(r_rank=100*(total-fund_ret_get['annROR'].rank(ascending=False)+1)*2/total) \
                                 .assign(v_1_rank=100*(total-fund_ret_get['avgRetrace'].rank(ascending=True)+1)*1/total)\
                                 .assign(v_2_rank=100*(total-fund_ret_get['Var'].rank(ascending=True)+1)*1.5/total)\
                                 .assign(v_3_rank=100*(total-fund_ret_get['maxRetrace'].rank(ascending=True)+1)*1/total)\
                                 .assign(q_rank=100*(total-fund_ret_get['IR'].rank(ascending=False)+1)*1/total)\
                                 .assign(rank=lambda df:df.r_rank+df.v_1_rank+df.v_2_rank+df.v_3_rank+df.q_rank)

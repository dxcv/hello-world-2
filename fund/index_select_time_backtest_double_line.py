...
if __name__ == '__main__':
    ...
    n = 1440
    # 单均线策略
    N1_lst=[3,5,10,20]  # 短均线参数
    N2_lst=[5,10,20,30,60,120]  # 短均线参数
    fee = 0.004
    symble_lst = 'A股指数'
...
#        .sort_values(['date_time'])
    ...
    for N1 in N1_lst:
        for N2 in N2_lst:                    
            print(symble_lst)
            method = 'double_line' +'_'+str(N1)            
            print(method)
            signal_lst = []
            trad_times = 0
            if (len(group) > N2) & (N1 < N2):
                net = 1
                net_lst = []
                group_ = group.copy()
                ...
                for idx, _row in group_.iterrows():
                    if (position == 0) & (_row.MA1_1 > _row.MA2_1):
                        position = 1
                ...
                plt.plot(net_lst,label= str(N1) + '_' + str(N2))
                ...
                signal_state.to_csv('result/signal_double_line_' + str(n) + '_' + method + '.csv')
    ...
    signal_state = pd.DataFrame(state_lst, columns=['symble', 'tm', 'win_r', 'odds', 'total_ret', 'ann_ret', 'sharp',
                                                    ...
                                                    'win_r_3', 'win_r_5', 'ave_max', 'n1','n2'])
    print(signal_state)
    signal_state.to_csv('result/state_double_line.csv')
...
def topsis(df,zb_lst,direc,w):
    ...
    G_tmp=tmp1-g_d
    tmp1['G']=G_tmp.apply(lambda x:w[0]*x['annROR']**2+w[1]*x['avgRetrace']**2+w[2]*x['Var']**2+w[3]*x['IR']**2,axis=1)
    B_tmp=tmp1-b_d
    tmp1['B']=B_tmp.apply(lambda x:w[0]*x['annROR']**2+w[1]*x['avgRetrace']**2+w[2]*x['Var']**2+w[3]*x['IR']**2,axis=1)
...
if __name__=='__main__':
      ...
      operate_mode_id=[401001, 401005, 401006]
      underlying_asset_type_id=[402001,402004,402005]
      # fund_id 为符合条件的基金名单
      num = 0
      for i in operate_mode_id:
          for j in underlying_asset_type_id:
                ...
                num += 1
      fund_id.to_csv('data/funds_zf.csv',encoding='gbk')
      ...
      df.to_csv('data/fund_value_zf.csv') 
      df=pd.read_csv('data/fund_value_zf.csv',index_col=0)
     # 计算每只基金评价指标 
#      fund_lst=fund_id.main_code.drop_duplicates().tolist()
    ...
    # 基金筛选
      fund_ret_get=fund_ret.query("annROR < 2")
      ...
      fund_ret_get.to_excel('result/fund_mark_all_topsis_zf'+'.xls',encoding='gbk')   
...
a = finance.run_query(query(finance.FUND_MAIN_INFO).filter(finance.FUND_MAIN_INFO.main_code=='501057'))
